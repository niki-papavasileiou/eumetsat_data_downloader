...
class DownloaderApp(QWidget):
    ...
    def stop_download(self):
        if self.collected_data_thread and self.collected_data_thread.is_alive():
            self.logger.info("Stopping the download process...")
            self.stop_event.set()  # Signal the thread to stop
            self.collected_data_thread.join(timeout=5)  # Avoid UI freezing
        self.btn_stop.setEnabled(False)
        self.btn_download.setEnabled(True)

    def closeEvent(self, event):
        """Forcefully kill all running threads and exit the application."""
        self.stop_event.set()  # Stop the download thread if running
        if self.collected_data_thread and self.collected_data_thread.is_alive():
...
if __name__ == "__main__":
    ...
    app.aboutToQuit.connect(downloader.closeEvent)
    downloader.show()
    sys.exit(app.exec_())
