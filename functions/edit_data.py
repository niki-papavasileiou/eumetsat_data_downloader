...
from functions.handle_files import delete_folder_contents
...
def edit_seviri(logger: logging, filename: str, selected_folder: str):
    ...
    for band in bands:
        ...
        if match:
            ...
            df.to_csv(file_path)
            # delete_folder_contents("MyData/SEVIRI")
...
def edit_fci(logger: logging, filename: str, selected_folder: str):
    ...
    bands = ["ir_87", "ir_105", "ir_123", "wv_63", "wv_73", "ir_38"]
    filename = [path_to_data[0]]
    ...
    for band in bands:
        ...
        match = re.match(r".*FCI.*_(\d{14})_(\d{14})", filename)
        if match:
            ...
            file_path = os.path.join(selected_folder, output_filename)
            df.to_csv(file_path)
            # delete_folder_contents("MyData/FCI")
...
def edit_fci_light(selected_folder: str):
    ...
    with Dataset(file_path[0], mode="r") as ds:
        ...
        df.to_csv(file_path)
        # delete_folder_contents("MyData/FCI_LI")
    ...
    if selected_collection in [
        ...
        if zipfile.is_zipfile(downloaded_file):
            with zipfile.ZipFile(downloaded_file, "r") as zip_ref:
                ...
                if selected_collection == "EO:EUM:DAT:MSG:HRSEVIRI":
                    edit_seviri(str(product))
                elif selected_collection == "EO:EUM:DAT:0662":
                    edit_fci(str(product))
                elif selected_collection == "EO:EUM:DAT:0691":
                    edit_fci_light(str(product))
